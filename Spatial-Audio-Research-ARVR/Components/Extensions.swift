...
extension Entity {
    static func createAxes(axisScale: Float, alpha: CGFloat = 1.0) -> Entity {
        ...
        let xAxis = ModelEntity(mesh: mesh, materials: [UnlitMaterial(color: .red.withAlphaComponent(alpha))])
        let yAxis = ModelEntity(mesh: mesh, materials: [UnlitMaterial(color: .green.withAlphaComponent(alpha))])
        let zAxis = ModelEntity(mesh: mesh, materials: [UnlitMaterial(color: .blue.withAlphaComponent(alpha))])
    ...
    static func createText(_ string: String, height: Float, color: Color = .white) -> ModelEntity {
...
extension HandAnchor {
    ...
    func nearestFingerDistance(to modelEntity: ModelEntity) -> (finger: Finger, distance: Float)? {
        guard isTracked,
              let bounds: BoundingBox = modelEntity.model?.mesh.bounds
        else {
            print("Failed to find entity or its bounds")
            return nil
        }
        ...
        return Finger.allCases.compactMap { finger in
            ...
            let localFingerPosition: SIMD3<Float> = modelEntity.convert(position: worldFingerPosition, from: nil)
